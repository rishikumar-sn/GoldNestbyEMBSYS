...
GALLERY_EXACT_MATCH_THRESHOLD = 0.975
...
NOT_GOLD_LABEL = "Not Gold Jewelry"
GALLERY_CLASS_THRESHOLDS = {
    "Bangle": 0.90,
    "Bracelet": 0.92,
    COMBINED_NECK_LABEL: 0.90,
    "Mattal": 0.92,
    "Earrings / Nosepin": 0.91,
    "Finger Ring": 0.91,
    OTHER_GOLD_LABEL: 0.94,
    NOT_GOLD_LABEL: 0.94,
}
PRODUCTION_LABELS = {
    "bangle": "Bangle",
    "bracelet": "Bracelet",
    "chain / necklace": COMBINED_NECK_LABEL,
    "chain": COMBINED_NECK_LABEL,
    "necklace": COMBINED_NECK_LABEL,
    "dollar": COMBINED_NECK_LABEL,
    "dollar chain": COMBINED_NECK_LABEL,
    "haram": COMBINED_NECK_LABEL,
    "kasu mala": COMBINED_NECK_LABEL,
    "kasu malai": COMBINED_NECK_LABEL,
    "mangalsutra": COMBINED_NECK_LABEL,
    "pendant": COMBINED_NECK_LABEL,
    "pendant chain": COMBINED_NECK_LABEL,
    "mattal": "Mattal",
    "earing / jumkha": "Earrings / Nosepin",
    "earring / jumkha": "Earrings / Nosepin",
    "earrings / nosepin": "Earrings / Nosepin",
    "earring": "Earrings / Nosepin",
    "earrings": "Earrings / Nosepin",
    "nose pin": "Earrings / Nosepin",
    "nosepin": "Earrings / Nosepin",
    "nose ring": "Earrings / Nosepin",
    "finger ring": "Finger Ring",
    "ring": "Finger Ring",
    "baby jewellery": OTHER_GOLD_LABEL,
    "other gold jewellery": OTHER_GOLD_LABEL,
    "not gold jewelry": NOT_GOLD_LABEL,
    "not gold jewellery": NOT_GOLD_LABEL,
}
...
def canonicalize_jewelry_label(label: str) -> str:
    cleaned = " ".join(str(label).split())
    return PRODUCTION_LABELS.get(cleaned.casefold(), cleaned)
...
class CorrectionGallery:
        ...
        if self.gallery_path.exists():
            try:
                with np.load(self.gallery_path, allow_pickle=True) as data:
                    ...
                    original_labels = [str(label) for label in data["labels"].tolist()]
                    labels = [canonicalize_jewelry_label(label) for label in original_labels]
                ...
                embeddings = _normalize_vector(embeddings)
                # Repeated confirmations of an almost identical crop must not
                # manufacture a false top-k consensus.
                keep: list[int] = []
                for index, (vector, label) in enumerate(zip(embeddings, labels, strict=False)):
                    duplicate = any(
                        labels[prior] == label and float(embeddings[prior] @ vector) >= 0.9995
                        for prior in keep
                    )
                    if not duplicate:
                        keep.append(index)
                self.embeddings = embeddings[keep]
                self.labels = [labels[index] for index in keep]
                # Normalize and deduplicate in memory. A read must not rewrite
                # the seed or race an atomic gallery replacement by another process.
    ...
    def add(self, embedding: np.ndarray, label: str, *, save: bool = True) -> None:
        normalized_label = canonicalize_jewelry_label(label)
        ...
        if (
            ...
            and self.embeddings.size > 0
            and self.embeddings.shape[1] != vector.shape[1]
        ):
            raise ValueError("Gallery embedding dimension does not match existing entries.")

        # Avoid storing effectively identical examples of the same class.
        if self.embeddings is not None and len(self.labels) > 0:
            same_label = np.array([item == normalized_label for item in self.labels])
            if same_label.any() and float(np.max(self.embeddings[same_label] @ vector[0])) >= 0.9995:
                return
    ...
    def search(
        ...
        threshold: float = GALLERY_THRESHOLD,
        expected_label: str | None = None,
    ) -> GallerySearchResult:
        if self.embeddings is None or len(self.labels) == 0:
            return GallerySearchResult(None, 0.0)
        ...
        top_indices = order[: min(GALLERY_TOP_K, len(order))]
        votes: dict[str, list[float]] = {}
        for index in top_indices:
            votes.setdefault(self.labels[int(index)], []).append(float(similarities[int(index)]))
        ranked = sorted(
            votes.items(),
            key=lambda item: (sum(item[1]) / len(item[1]), max(item[1]), len(item[1])),
            reverse=True,
        )
        winner, winner_scores = ranked[0]
        winner_similarity = max(winner_scores)
        runner_score = max(ranked[1][1]) if len(ranked) > 1 else -1.0
        margin = winner_similarity - runner_score
        support = len(winner_scores)
        exact_match = best_similarity >= GALLERY_EXACT_MATCH_THRESHOLD
        class_threshold = max(threshold, GALLERY_CLASS_THRESHOLDS.get(winner, threshold))
        consensus_match = support >= 2 and winner_similarity >= class_threshold and margin >= 0.005
        disagrees = bool(expected_label and winner != canonicalize_jewelry_label(expected_label))
        if disagrees and not exact_match:
            consensus_match = bool(
                support >= 3
                and winner_similarity >= class_threshold + GALLERY_DISAGREEMENT_MARGIN
                and margin >= GALLERY_DISAGREEMENT_MARGIN
            )
        if exact_match:
            winner = self.labels[best_index]
            support = sum(1 for index in top_indices if self.labels[int(index)] == winner)
        if exact_match or consensus_match:
            return GallerySearchResult(winner, winner_similarity, support, margin)
        return GallerySearchResult(None, best_similarity, support, margin)
...
class JewelryZeroShotClassifier:
    def __init__(
        ...
        requested_providers = providers or ["CPUExecutionProvider"]
        active_providers = [
    ...
    def learn_correction(self, image_path: str | Path, label: str) -> None:
        ...
        self.gallery.add(self.embedding_for_image(image), label)

    def embedding_for_image(self, image: Image.Image) -> np.ndarray:
        """Return the same normalized embedding used by production matching."""
        original = image.convert("RGB")
        cropped = self._crop_foreground(original)
        merged = (self._embed_image(original) + self._embed_image(cropped)) / 2.0
